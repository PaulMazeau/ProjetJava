...
public class Controller implements Initializable
{
	...
	/**
	 * Toolbar "Redo" Button
	 * @implSpec Should be part of <label fx:id="undButton"/>
	 */
	@FXML
	...
	public void initialize(URL location, ResourceBundle resources)
	{
		...
		historyManager = new HistoryManager<Figure>(???, ???, ???);
...
//		lineTypeCombobox.setValue(LineType.SOLID);
		...
		lineWidthSpinner.setValueFactory(new SpinnerValueFactory.DoubleSpinnerValueFactory( ???, ???, ???, ???));
	...
	protected void setTools(boolean editMode)
	{
		...
		if (editMode) {
			currentTool = new SelectionTool(...); transformTool = new TransformTool(...);
		} else { // creation mode
